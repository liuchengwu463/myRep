...
cookie_file_path = os.path.join("../cookie", "cookie4.txt")
...
day = now.strftime("%d")
file_path = os.path.join('..', month, day, "num4.txt")
...
for index, bill_id in enumerate(bill_id_list):
    ...
    if len(sign_find_result) == 0:
        with open(os.path.join(wait_decrypt_path, "unsign_bill_id4.txt"), 'a', encoding='utf-8') as f:
            f.write(f'{bill_id}\n')
        print(f'{bill_id} 未签收')
        continue

    with open(os.path.join(wait_decrypt_path, "sign_num4.txt"), 'a', encoding='utf-8') as f:
        f.write(f'{bill_id}\n')
