...
for index, bill_id in enumerate(bill_id_list):
    ...
    if len(sign_find_result) == 0:
        with open(os.path.join(wait_decrypt_path, "unsign_bill_id1.txt"), 'a', encoding='utf-8') as f:
            f.write(f'{bill_id}\n')
        print(f'{bill_id} 未签收')
        continue

    with open(os.path.join(wait_decrypt_path, "sign_num1.txt"), 'a', encoding='utf-8') as f:
        f.write(f'{bill_id}\n')
